...
from pathlib import Path  # Добавьте эту строку в самом начале файла
...
def create_app():
    ...
    # Добавьте конфигурацию БД
#    local_app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///instance/mydatabase.db'
    db_path = Path(__file__).parent / 'instance' / 'mydatabase.db'
    local_app.config['SQLALCHEMY_DATABASE_URI'] = f'sqlite:///{db_path}'
...
def __handle_mqtt_message_iotdatahub(
    ...
    try:
        ...
        mqtt.publish(f'{data.topic}/status', f"{e}".encode())
        return
