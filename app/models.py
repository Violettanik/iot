...
class User(db.Model, fsqla.FsUserMixin, UserMixin, BasicDatahubMixin):
    access_level = db.Column(
        db.Integer,  # Храним как целое число
        default=AccessLevel.STUDENT.value,
        nullable=False
    )
    
    @property
    def access_level_enum(self):
        return AccessLevel(self.access_level)
    
    @access_level_enum.setter
    def access_level_enum(self, value):
        self.access_level = value.value
